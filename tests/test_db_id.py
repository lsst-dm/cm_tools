...
def test_db_id() -> None:
    ...
    db_top_id = DbId()
    assert db_top_id.to_tuple() == (None, None, None, None)
    assert db_top_id.level() is None

    db_p_id = db_top_id.extend(LevelEnum.production, 0)
    assert db_p_id.to_tuple() == (0, None, None, None)
    ...
    assert db_c_id.to_tuple() == (0, 0, None, None)
    ...
    assert db_s_id.to_tuple() == (0, 0, 0, None)
    ...
    assert db_g_id.to_tuple() == (0, 0, 0, 0)
